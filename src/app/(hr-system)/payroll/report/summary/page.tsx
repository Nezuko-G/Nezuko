...
const StatCard = ({ label, value, accent }: { label: string; value: number; accent?: boolean }) => (
  <div className={`rounded-2xl border p-5 ${accent ? 'bg-secondary border-secondary-hover text-white' : 'bg-white border-gray-100'}`}>
    <p className={`text-xs font-semibold uppercase tracking-wide mb-1 ${accent ? 'text-gray-400' : 'text-content-muted'}`}>{label}</p>
    <p className={`text-2xl font-black ${accent ? 'text-primary' : 'text-content-dark'}`}>{formatCurrency(value)}</p>
  </div>
);
    ...
    <div className="min-h-screen bg-background p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <PageHeader
          ...
          action={
            report ? (
              <button
                onClick={exportCSV}
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-secondary hover:bg-secondary-hovert-semibold text-sm transition-colors"
              >
        ...
        <div className="flex flex-wrap items-end gap-3 mb-8 bg-white border border-gray-100 rounded-2xl p-4">
          <div>
            <label className="block text-xs font-semibold text-content-muted mb-1">Month</label>
            <select
              value={month}
              onChange={(e) => setMonth(Number(e.target.value))}
              className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-content-dark focus:outline-none focus:ring-2 focus:ring-primary transition"
            >
          ...
          <div>
            <label className="block text-xs font-semibold text-content-muted mb-1">Year</label>
            <select
              value={year}
          ...
          <>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 className="font-bold text-content-dark">
                  {formatMonthYear(report.month, report.year)} — By Department
                </h2>
                <span className="text-xs text-content-muted">{report.grandTotal.headCount} employees</span>
              </div>
                  ...
                  <tbody>
                    {report.departments.map((dept) => (
                      <tr key={dept.departmentId} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                        <td className="px-4 py-3.5 font-semibold text-content-dark">{dept.departmentName}</td>
                        <td className="px-4 py-3.5 text-content-muted">{dept.headCount}</td>
                        <td className="px-4 py-3.5 text-content">{formatCurrency(dept.totalBaseSalary)}</td>
                        ...
                        <td className="px-4 py-3.5 font-bold text-primary-hover">{formatCurrency(dept.totalNet)}</td>
                      </tr>
                    ))}
                    {/* Grand Total row */}
                    <tr className="bg-secondary text-white">
                      ...
                      <td className="px-4 py-3.5 font-bold text-primary">{formatCurrency(report.grandTotal.totalOvertimePay)}</td>
                      <td className="px-4 py-3.5 font-bold text-primary">{formatCurrency(report.grandTotal.totalIncentives)}</td>
                      <td className="px-4 py-3.5 font-bold text-red-300">{formatCurrency(report.grandTotal.totalDeductions)}</td>
                      <td className="px-4 py-3.5 font-black text-primary">{formatCurrency(report.grandTotal.totalNet)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
