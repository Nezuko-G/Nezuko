    ...
    <div className="min-h-screen bg-background p-6 lg:p-8 flex flex-col gap-4">
      <div className="max-w-7xl mx-auto w-full flex flex-col gap-4">
        ...
        <div className="w-25 max-w-xs ">
          ...
          >
            <option value="ALL">{t("runs.filters.ALL")}</option>
            <option value="DRAFT">{t("runs.filters.DRAFT")}</option>
            <option value="APPROVED">{t("runs.filters.APPROVED")}</option>
            <option value="PAID">{t("runs.filters.PAID")}</option>
          </select>
        </div>
                          ...
                          <td className="px-4 py-3.5">
                            <div className="flex items-center gap-2">
                              ...
                              {run.status === "DRAFT" && (
                                <button
                                  onClick={() =>
                                    setConfirmState({ type: "approve", run })
                                  }
                              ...
                              {run.status === "APPROVED" && (
                                <button
                                  onClick={() =>
                                    setConfirmState({ type: "markPaid", run })
                                  }
      ...
      <ConfirmModal
        ...
        onConfirm={
          confirmState?.type === "approve" ? handleApprove : handleMarkPaid
        }
        title={
          confirmState?.type === "approve"
            ? t("runs.confirm.approveTitle", {
                monthYear: confirmState.run
                  ? formatMonthYear(
                      confirmState.run.month,
                      confirmState.run.year,
                      t,
                    )
                  : "",
              })
            : t("runs.confirm.markPaidTitle", {
                monthYear: confirmState?.run
                  ? formatMonthYear(
                      confirmState.run.month,
                      confirmState.run.year,
        ...
        confirmLabel={
          confirmState?.type === "approve"
            ? t("runs.confirm.approveBtn")
            : t("runs.confirm.markPaidBtn")
        }
      />
    </div>
  );
